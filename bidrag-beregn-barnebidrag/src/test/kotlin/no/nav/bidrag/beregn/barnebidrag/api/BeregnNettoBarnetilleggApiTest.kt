...
internal class BeregnNettoBarnetilleggApiTest {
    ...
    fun testNettoBarnetillegg_Eksempel01() {
        ...
        val resultat = utførBeregningerOgEvaluerResultatNettoBarnetillegg()
    ...
    @Test
    @DisplayName("Netto barnetillegg - eksempel 2  ")
    fun testNettoBarnetillegg_Eksempel02() {
        ...
        assertAll(
            ...
            { assertEquals(0, resultat[0].summertBruttoBarnetillegg.compareTo(BigDecimal.valueOf(2000.00))) },
            { assertEquals(0, resultat[0].summertNettoBarnetillegg.compareTo(BigDecimal.valueOf(1315.00))) },

            { assertEquals(0, resultat[0].barnetilleggTypeListe[0].barnetilleggType.compareTo(Barnetilleggstype.PENSJON)) },
            { assertEquals(0, resultat[0].barnetilleggTypeListe[0].bruttoBarnetillegg.compareTo(BigDecimal.valueOf(1700.00))) },
            { assertEquals(0, resultat[0].barnetilleggTypeListe[0].nettoBarnetillegg.compareTo(BigDecimal.valueOf(1105.00))) },

            { assertEquals(0, resultat[1].barnetilleggTypeListe[0].barnetilleggType.compareTo(Barnetilleggstype.PENSJON)) },
            { assertEquals(0, resultat[1].barnetilleggTypeListe[0].bruttoBarnetillegg.compareTo(BigDecimal.valueOf(300.00))) },
            { assertEquals(0, resultat[1].barnetilleggTypeListe[0].nettoBarnetillegg.compareTo(BigDecimal.valueOf(210.00))) },
        )
    }


    private fun utførBeregningerOgEvaluerResultatNettoBarnetillegg(): List<DelberegningNettoBarnetillegg> {
        val request = lesFilOgByggRequest(filnavn)
        val nettoBarnetilleggResultat = beregnBarnebidragService.beregnNettoBarnetillegg(request)
